...
/**
 * @brief CallBack class : Remove Dynamic Memory Module
 *
 * @details This class is soley reponsible for removing all dynamic memory allocations in the input
 * source code and replacing them with static memory.
 * The following source code transformations are performed by this class/module:
 * - Replacement of void* declaration with array declaration.
 * - Removal of the calloc() call.
 * - Removal of the free() call.
 */
class MakeStaticMatchCallback : public BaseMatchCallback {
  public:
     ...
     *
     * @param call_expr  The call expression, representing the call to free()
     */
     ...
     *
     * @param assign_expr  This is a binary operator, which is supposed to be
     ...
     *
     * @param var_decl  The declaration of the pointer variable.
     *
     * @param type  The type of the array that will replace the pointer variable,
     *              extracted from the removed calloc() call.
     *
     * @param count_src  The number of elements in the array that will replace the
    ...
    /**
     * @return Number of calls to free() processed by the class.
     */
    inline unsigned int num_free_calls() const { return _num_free_calls; }

    /**
     * @return Number of calls to calloc() processed by the class.
     */
    inline unsigned int num_calloc_calls() const { return _num_calloc_calls; }

    /**
     * @return Number of pointer variables that were replaced with array variables.
     */
    inline unsigned int num_variable_replacements() const { return _num_variable_replacements; }

    /**
     * Set: Code modification occurs by removing the code. Default operation.
     */
    inline void remove_code() { _remove_code = true; }

    /**
     * Set: Code modification occurs by commenting out the code.
     */
  ...
  private:
    ...
    /// If this variable is set to true, then the improper code constructs are deleted entirely.
    /// If this variable is set to true, then the improper code constructs are simply commented out.
