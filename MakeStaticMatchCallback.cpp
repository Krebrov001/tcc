...
void MakeStaticMatchCallback::remove_free(const CallExpr* call_expr)
{
    ...
    // Comment out the unneeded code (as opposed to remove it)
        ...
        Replacement free_start_rep(*SM, loc_start, 0, "/*");
...
void MakeStaticMatchCallback::remove_calloc(const BinaryOperator* assign_expr)
{
    string oper_string = assign_expr->getOpcodeStr();
    if (oper_string != "=") {
        outputSource(assign_expr, errs());
        errs() << "ERROR: The binary operator is not an assignment.\n";
        errs() << "\n\n";
        return;
    }
    ...
    bool count_invalid;
    string count_src = Lexer::getSourceText(
        CharSourceRange::getTokenRange(count_expr->getSourceRange()),
        *SM, LangOptions(), &count_invalid
    ).str();
    // count_invalid is modified by CharSourceRange::getTokenRange() to mark if that operation
    // succeeded.
    if (count_invalid) {
        outputSource(ce, errs());
    ...
    SourceLocation loc_start = assign_expr->getBeginLoc();
    ...
    if (_remove_code) {
        ...
        if (Error err = (*replacements)[calloc_rep.getFilePath()].add(calloc_rep)) {
            outputSource(assign_expr, errs());
            errs() << "ERROR: Error adding replacement that removes calloc call and assignment.\n";
            errs() << "\n\n";
            return;
        }
    ...
    // Comment out the unneeded code (as opposed to remove it)
        ...
        Replacement calloc_start_rep(*SM, loc_start, 0, "/*");
    ...
    replace_variable_type(var_decl, type, count_src);
}


void MakeStaticMatchCallback::replace_variable_type(
    const DeclaratorDecl* var_decl, const string& type, const string& count_src
) {
    SourceLocation loc_start = var_decl->getBeginLoc();
    SourceLocation loc_end   = var_decl->getEndLoc();

    string original_type = var_decl->getType().getAsString();

    // Change the variable type (typically void*) to type specified in call to sizeof()
    Replacement type_rep(*SM, loc_start, original_type.length(), type + " ");
    ...
    Replacement array_rep(*SM, loc_end, var_decl_name.length(), var_decl_name + array_src);
