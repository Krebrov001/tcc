...
unique_ptr<ASTConsumer> StaticAnalysisAction::CreateASTConsumer(
    ...
    // This is not an AnalyzerOptions object, it is actually a reference to a data structure
    // which is a part of the CompilerInstance. Modifications to AnalyzerOptionsRef affect the
    // original CompilerInstance.
    ...
    // information.
    // The reason the Compiler is passed into CreateAnalysisConsumer() is because it contains
    // a data structure which is the AnalyzerOptions storing the list of checkers to run.
