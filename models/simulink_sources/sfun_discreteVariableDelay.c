 ...
 * Copyright 1997-2016 Hydro-Quebec, and The MathWorks, Inc.
 * -----------------------------------------------------------------------------
 */
...
#define ERR_INVALID_DELAY_SIZE \
    "The 'D' input size must be 1 or equal to the 'In' input size"
#define ERR_INVALID_INIT_COND_SIZE \
    "The initial condition size must be 1 or equal to the 'In' input size"
...
static void
mdlSetOutputPortWidth(SimStruct* S, int_T port, int_T outputPortWidth)
{
 ...
 * =========================================== Abstract: In case no ports were
 * specified, the default is an input port of width 2 and an output port of
 * width 1.
 */
...
static void mdlStart(SimStruct* S)
{
    ...
    if (initValueSize > bufSize) {
        ...
        mexWarnMsgIdAndTxt(
            ...
            "Block %s: Delay buffer has been increased to %d samples, in order to respect the initial condition length.",
            ssGetPath(S),
            initValueSize);
#endif
    }
...
static void mdlInitializeConditions(SimStruct* S)
{
    ...
    for (i = 0; i < uWidth; i++) {
        ...
        if ((initValueNbr == 1) && (initValueSize == 1)) {
            ...
            memcpy(&buffer[i][bufSize - initValueSize],
                   &initValues[0],
                   initValueSize * sizeof(real_T));
        } else {
            memcpy(&buffer[i][bufSize - initValueSize],
                   &initValues[i * initValueSize],
                   initValueSize * sizeof(real_T));
        }
        idxWrite[i] = 0;
    }
...
static void mdlOutputs(SimStruct* S, int_T tid)
{
    ...
    InputRealPtrsType uPtrs =
        ssGetInputPortRealSignalPtrs(S, (directFeedThrough == 1 ? 0 : 1));
    ...
    for (i = 0; i < uWidth; i++) {
        ...
        /* Correct the possible imprecision when near a multiple of the time
         * step */
        d = d + mxGetEps();
        ...
        if ((directFeedThrough == 0) && (d < Ts)) {
            /* Set the delaySmallerThanTsFLAG to 1 so we can throw a warning in
             * mdlTerminate */
        ...
        i, idxWrite, buffer[i][idxWrite], d, (int_T)mod(idxRead, bufSize),
        (int_T)mod(idxRead-1, bufSize), output[i]);
         */
    }
}
...
static void mdlTerminate(SimStruct* S)
{
    ...
    if (pinfo->delaySmallerThanTsFLAG == 1) {
        ...
        mexWarnMsgIdAndTxt(
            "SpecializedPowerSystems:DiscreteVaraiableTimeDelay:InvalidParameter",
            "Block %s: The specified delay time is smaller than the specified Sample time (Ts). This may cause inaccurate simulation results.",
            ssGetPath(S));
#endif
    }
    ...
    if (pinfo->delayGreaterThanMaxDelayFLAG == 1) {
        ...
        mexWarnMsgIdAndTxt(
            "SpecializedPowerSystems:DiscreteVaraiableTimeDelay:InvalidParameter",
            "Block %s: The delay used is greater than the specified maximum delay parameter (%g). Limiting delay value to maximum delay.",
            ssGetPath(S),
            maxDelay);
#endif
    }
