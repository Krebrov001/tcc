...
/**
 * @brief Static Analysis API : Action object
 *
 * @details Instances of the StaticAnalysisAction get created by the StaticAnalysisActionFactory.
 * Each such instance is responsible for processing a single translation unit.
 * To do this, it creates a StaticAnalysisDiagnosticConsumer to collect diagnostics from the
 * Static Analyzer.
 *
...
class StaticAnalysisAction : public ASTFrontendAction {
  ...
  public:
     ...
     * This reference to the vector is temporarily stored as a data member of the class, and when a
     * StaticAnalysisDiagnosticConsumer is created, the reference to the vector is transferred to it.
     */
     ...
     *
     * @param Compiler  Helper class for managing a single instance of the Clang
     ...
     *
     * @param File  The current file that is being processed. A reference to a string (LLVM class).
     *
